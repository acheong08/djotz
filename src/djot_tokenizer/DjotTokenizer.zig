...
pub fn BuildInlineDjotTokens(allocator: std.mem.Allocator, doc: []const u8, parts: std.ArrayList(Range)) ![]Tokens {
    if (parts.items.len == 0) {
        parts.append(Range{ .start = 0, .end = doc.len });
    }
    ...
    tokenStack.openLevelAt(Token.init(Tokens.ParagraphBlock, leftDocumentPosition, leftDocumentPosition));

    var inlineParsingLoop = true;

    var reader = TextReader.init(doc);
    var state = parts.items[0].start;
    tokenStack.lastLevel().?.fillUntil(parts.items[0].start, Tokens.Ignore);
    for (parts.items) |part| {
        if (inlineParsingLoop == true) {
            reader = TextReader.init(doc);
            state = part.start;
            tokenStack.lastLevel().?.fillUntil(parts.items[0].start, Tokens.Ignore);
        }
        inlineParsingLoop = true;
        while (!reader.isEmpty(state)) {
            ...
            if (openInlineType == Tokens.VerbatimInline) {
                ...
                if (std.mem.trimLeft(u8, openToken, "$") != closeToken) {
                    state = next;
                    continue;
                }
                tokenStack.closeLevelAt(Token.init(Opposite(Tokens.VerbatimInline), state, next));
                state = next;
                continue;
            }
            ...
            inline for ([_]Tokens{ Tokens.EscapedSymbolInline, Tokens.SmartSymbolInline }) |tokenType| {
                if (try matchInlineToken(reader, state, tokenType)) |next| {
                    tokenStack.lastLevel().?.push(Token.init(tokenType, state, next));
                    state = next;
                    inlineParsingLoop = false;
                    break;
                }
            }
            inline for ([_]Tokens{
                ...
                const next = matchInlineToken(reader, state, Opposite(tokenType));
                const forbidClose = ((tokenType == Tokens.EmphasisInline and lastInline.tokenType == Tokens.EmphasisInline) or (tokenType == Tokens.StringInline and lastInline.tokenType == Tokens.StrongInline)) and lastInline.end == state;
                if (!forbidClose and next != null and try tokenStack.popForgetUntil(tokenType)) {
                    tokenStack.closeLevelAt(Token.init(Opposite(tokenType), state, next));
                    state = next;
                    inlineParsingLoop = false;
                    break;
                }
                ...
                if (matchInlineToken(reader, state, tokenType)) |nnext| {
                    attributes = Attributes.init(allocator);
                    const token = reader.doc[state..nnext];
                    if (tokenType == Tokens.VerbatimInline) {
                        if (std.mem.startsWith(u8, token, "$$")) {
                            attributes.set(djotToken.blockKeys.DisplayMathKey, "");
                        } else if (std.mem.startsWith(u8, token, "$")) {
                            attributes.set(djotToken.blockKeys.InlineMathKey, "");
                        }
                    }
                    const tok = Token.init(tokenType, state, nnext);
                    tok.attributes = attributes;
                    tokenStack.openLevelAt(tok);
                    state = nnext;
                    inlineParsingLoop = false;
                    break;
                }
    ...
    if (tokenStack.lastLevel().?.firstOrDefault().tokenType == Tokens.VerbatimInline) {
        tokenStack.closeLevelAt(Token.init(Opposite(Tokens.VerbatimInline), rightDocumentPosition, rightDocumentPosition));
    }
    tokenStack.popForgetUntil(Tokens.ParagraphBlock);
    tokenStack.closeLevelAt(Token.init(Tokens.ParagraphBlock, rightDocumentPosition, rightDocumentPosition));
    const tokens = tokenStack.lastLevel().?.*;
